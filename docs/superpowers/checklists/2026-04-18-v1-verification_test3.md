...
- [正确，通知栏里的PIN引导用户去APP查看，通知栏里不显式具体的PIN码] 通知栏出现常驻通知，显示 URL 和 PIN
...
- [正确] 手机端输入消息并点发送：浏览器收到；手机气泡视觉上区分（origin=PHONE）
- [正确] 电脑上传 1 MB 文件：上传成功，手机消息列表多出一条文件消息，APP 底部文件计数 +1
- [正确] 手机点"附件"选文件：浏览器收到文件提示气泡，点击可下载
- [正确] 浏览器顶部状态：uptime 每秒 +1、fileCount 正确、rate 非零
...
记录验证日期与设备：4-18-test3-Android15
...
## 2026-04-18 基于 test3 反馈的补充修复

- **通知栏 PIN 安全性调整**：用户反馈锁屏通知不应显示具体 PIN 值，仅 URL 可显示。改 `TransferService` 文案为 `"URL  http://IP:PORT / PIN  打开 APP 查看"`。原因见 `docs/notes/decisions.md` D9。

test3 其余条目全部通过，v1 可以定版。
