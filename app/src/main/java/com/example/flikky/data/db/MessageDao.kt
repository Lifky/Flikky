...
interface MessageDao {
    ...
    /**
     * Largest message id currently persisted (0 if the table is empty). Used at
     * app start to seed [com.example.flikky.util.IdGen]'s process-volatile
     * counter above all existing ids — otherwise a cold restart re-mints
     * 1,2,3… and collides with these PRIMARY KEYs. `MessageEntity.id` is
     * non-autogenerated (ids come from IdGen across phone + browser), so the
     * app owns id allocation and must not restart it from 0.
     */
