...
class AttachSheetSourcesTest {
    ...
    @Test fun `split apps are warned about before sending`() {
        val p = code("AppPickerContent.kt")
        assertTrue(p.contains("R.string.apps_split_warning_title"))
        assertTrue(Regex("""if\s*\(\s*app\.splitCount\s*>\s*0\s*\)\s*splitTarget\s*=\s*app\s*else\s*onSend\s*\(\s*app\s*\)""").containsMatchIn(p))
    }
