...
- **v1.13.0** — *released (2026-07-05)* — **optional PIN auth & local favorites**: the **连接需要 PIN 码** setting can now be turned off — with it disabled the embedded server drops the auth gate so any device on the same Wi-Fi reaches the address directly (still bound to the Wi-Fi IPv4 only, never `0.0.0.0`); left on, the single-use 6-digit PIN behaves exactly as before. The gate is centralized in a new `AuthGate` (`required` flag + token validation) that every route consults, so toggling it live re-gates every endpoint at once. Favorites gain **local creation** independent of any session: an 添加收藏 sheet stores a typed text snippet (`addLocalText`) or a picked local file (`addLocalFile`, copied into the favorites depot) straight into a collection, using a reserved local source-session id so these entries coexist with session-sourced favorites and survive on their own. Plus UI polish: the add-favorite sheet is simplified, settings rows center their trailing accessories, and the PIN row uses the `ic_fiber_pin` glyph. App `versionName` / `versionCode` → 1.13.0 / 11300.
...
- **Single-use PIN (optional).** By default authentication burns the PIN; subsequent attempts need a new PIN. 3 wrong attempts lock the IP for 30 s; 5 wrong attempts terminate the service. The PIN gate can be turned off in settings (**连接需要 PIN 码**), trading it for open access to anyone on the same Wi-Fi *(v1.13.0)*.
